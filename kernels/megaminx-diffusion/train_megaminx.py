...
PZ_WALL_CAP (default 7200s = 2h - stop training early so the solve phase
gets the remaining kernel budget).
"""
...
WALL_CAP = float(os.environ.get("PZ_WALL_CAP", "5400"))  # 0 = no cap
...
class TimedTrainer(Trainer):
    ...
    def run(self):
        ...
        for epoch in range(self.num_epochs):
            self.epoch += 1
            X, Y = self.generate_random_walks(k=self.walkers_num,
            ...
            if (self.epoch & (self.epoch - 1)) == 0:
                ...
                print(f"[{time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())}] "
                      f"Saved weights at epoch {self.epoch:5d}.", flush=True)
            if WALL_CAP > 0 and (time.time() - t_start) > WALL_CAP:
                print(f"wall cap {WALL_CAP:.0f}s reached at epoch {self.epoch} - stopping",
                      flush=True)
                break
        weights_file = f"{self.weights_dir}/{self.name}_{self.id}_e{self.epoch:05d}.pth"
        torch.save(self.net.state_dict(), weights_file)
...
def main():
    ...
    print(f"generators={n_gens} state_size={state_size} classes={num_classes} "
          f"Kmax={KMAX} wall_cap={WALL_CAP:.0f}s", flush=True)

    model = Pilgrim(num_classes=num_classes, state_size=state_size,
                    hd1=HD1, hd2=HD2, nrd=NRD, dropout_rate=0.0)
    model.to(device)
    if int(V0.min()) < 0:
        model.z_add = -int(V0.min())

    trainer = TimedTrainer(net=model, num_epochs=EPOCHS, device=device,
                           batch_size=BATCH, lr=0.001,
                           name=f"p{GROUP_ID:03d}-t{TARGET_ID:03d}",
                           K_min=1, K_max=KMAX,
    ...
    info = {"model_id": trainer.id, "hd1": HD1, "hd2": HD2, "nrd": NRD,
            "dropout": 0.0, "epochs": trainer.epoch, "K_max": KMAX}
    with open(info_path, "w") as f:
        json.dump(info, f)
    print("training complete. model_id=", trainer.id, "epochs=", trainer.epoch,
          flush=True)
